        ...
        for v in docs[k].keys():
            if docs[k][v][0] == "CREATE" or docs[k][v][0] == "UPDATE":
                if docs[k][v][1] == 'http://publications.europa.eu/ontology/cdm#work':
                    query_string = WORK_TEMPLATE % {
                        'uri': v.replace('cellar:', 'http://publications.europa.eu/resource/cellar/')}
                if docs[k][v][1] == 'http://publications.europa.eu/ontology/cdm#expression':
                    query_string = EXPRESSION_TEMPLATE % {
                        'uri': v.replace('cellar:', 'http://publications.europa.eu/resource/cellar/')}
                if docs[k][v][1] == 'http://publications.europa.eu/ontology/cdm#manifestation':
                    query_string = MANIFESTATION_TEMPLATE % {
                        'uri': v.replace('cellar:', 'http://publications.europa.eu/resource/cellar/')}
                sparql_result = sparql_query(query_string)
                if sparql_result != []:
                    docs[k][v].append(
                        post_process[docs[k][v][1]](sparql_result))
                else:
                    print(v + " is empty")
                    docs[k][v].append({})
            else:
...
def post_process_work_result(result):
    ...
    identifiers = result[0]['identifiers'][
        'value'].replace(", ", ",").split(',')
    resource_types = result[0]['resource_types'][
        'value'].replace(", ", ",").split(',')
            ...
            if jsonDocument.get('manifestations') != None:
                if attachment_key in registered_format.keys():
                    if index_prefs[registered_format[attachment_key][0]] > index_prefs[attachment_format]:
                        jsonDocument['manifestations'][
                            registered_format[attachment_key][1]] = doc[k][3]
                else:
                    jsonDocument['manifestations'].append(doc[k][3])
                    registered_format[attachment_key] = (
                        doc[k][3]['type'], len(jsonDocument['manifestations']) - 1)
            else:
                jsonDocument['manifestations'] = [doc[k][3]]
                registered_format[attachment_key] = (
                    doc[k][3]['type'], len(jsonDocument['manifestations']) - 1)
    #print (json.dumps(jsonDocument, indent=4, sort_keys=True))
    return jsonDocument
...
def add_attachment(jsonDocument):
    ...
    if 'manifestations' in jsonDocument.keys():
        for m in jsonDocument['manifestations']:
            ...
            manifestation = {
                'uri': m['uri'], 'type': m['type'], 'text_' + m['lang'].lower(): attachment}
...
def get_attachment(uri):
    headers = {
        "Accept": "text/html,application/xhtml+xml,application/pdf;type=pdfa1a"}
    # because bigdata machines cannot resolve publications.europa.eu,
    # we have to intercept and modify the redirect.
    resp = requests.get(uri.replace(
        'publications.europa.eu', 'opsrv752:8180'), headers=headers, allow_redirects=False)
    redirect_uri = resp.headers['location']
    resp = requests.get(redirect_uri.replace(
        'publications.europa.eu', 'opsrv752:8180'), headers=headers, allow_redirects=False)
